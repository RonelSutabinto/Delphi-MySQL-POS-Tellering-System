...
uses
  ...
  MemDS, StdCtrls, Mask, DBCtrls, NxEdit, ExtCtrls, CRDBGrid1, QExport4,
  QExport4XLS;
...
begin
  ...
  Deposit2.MacroByName('depositdate').Active := True;
  Deposit2.ParamByName('DateFrom').AsDate := DateFrom.Date;
  Deposit2.ParamByName('DateTo').AsDate   := DateTo.Date;
  Deposit2.Open;
end;
...
procedure TDepositSlipForm.NxButton5Click(Sender: TObject);
begin
    QExport4XLS1.FileName := ExtractFilePath(Application.ExeName) + 'Deposit2.xls';
...
begin
  If RadioGroup1.ItemIndex = 0 then
    begin
      ...
      Deposit2.MacroByName('depositdate').Active := True;
      Deposit2.MacroByName('dcprdate').Active    := False;
    end else
    begin
      Deposit.MacroByName('depositdate').Active := False;
      Deposit.MacroByName('dcprdate').Active    := True;

      Deposit2.MacroByName('depositdate').Active := False;
      Deposit2.MacroByName('dcprdate').Active    := True;
    end;
end;
