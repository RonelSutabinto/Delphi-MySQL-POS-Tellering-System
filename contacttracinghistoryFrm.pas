...
uses
  ...
  Vcl.DBGrids, CRGrid, NxCollection, NxEdit;
...
type
  TcontacttracingFrm = class(TForm)
    ...
    TellerEff: TNxButton;
    NxButton1: TNxButton;
    cntcthistoryQpriority: TStringField;
    fromdate: TNxDatePicker;
    todate: TNxDatePicker;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
procedure TcontacttracingFrm.FormShow(Sender: TObject);
begin
  fromdate.Date := now;
  todate.Date   := now;
end;
...
begin
    ...
    cntcthistoryQ.ParamByName('fromdate').Text := FormatDateTime('YYYY-MM-DD', fromdate.Date);
    cntcthistoryQ.ParamByName('todate').Text   := FormatDateTime('YYYY-MM-DD', todate.Date);
    cntcthistoryQ.Open;
end;
